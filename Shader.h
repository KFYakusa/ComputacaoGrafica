...
public:
	...
	static bool createProgram(GLuint* ID, std::string arqVertexShader, std::string arqFragShader) {
		...
		if (!Shader::createShader(GL_VERTEX_SHADER, arqVertexShader, &vShader)) {
			std::cout << "criarPrograma: Erro ao criar o vertex shader!" << std::endl;
			return false;
		}
		...
		if (!Shader::createShader(GL_FRAGMENT_SHADER, arqFragShader, &fShader)) {
			std::cout << "criarPrograma: Erro ao criar o fragment shader!" << std::endl;
			return false;
		}
		...
		//libera memoria dos shaders (eles j� foram compilados e anexados ao programa)
	...
	static bool createShader(int type, std::string path, GLuint* ID) {
...
private:
	...
	static void printProgramInfoLog(GLuint progama) {
		int len = 0;
		int chWrittn = 0;
		char* log;
	...
	static void printShaderInfoLog(GLuint shaderRef) {
		int len = 0;
		int chWrittn = 0;
		char* log;
